...
if __name__ == '__main__':
    ...
    # df = pd.DataFrame()
    # df = pd.read_csv("/Users/xinyi/PycharmProjects/TCO_ex3/data/"+file_name+".csv", dtype={"time": float, "rate": float})
    # best_solution, best_energy = get_initial_fval(len(df))
    # OrderByImpactNum(best_solution, df, best_energy)
    # OrderByImpact(best_solution, df, best_energy)
    num_experiment = 10
    reps = 1
    problem_size = 7
    df = pd.DataFrame()
    ...
    while count < 10:
        ...
        if problem_size>0.15*len(df):
            ...
            result_fval = qubo.objective.evaluate(bitstring) # fitness values of all subproblems
            fval_list.append(result_fval)
        ...
        if not os.path.exists("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)):
            os.makedirs("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment))
        log_df.to_csv("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/log.csv")

        if not os.path.exists("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)):
            os.makedirs("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment))
        result_df.to_csv("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/itr_results.csv")

        if not os.path.exists("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)):
            os.makedirs("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment))
        solution_df.to_csv("../results/igdec_qaoa/noise/qaoa_"+str(reps)+"/" + file_name + "/size_" + str(problem_size) + "/" + str(num_experiment)+"/solution.csv")
